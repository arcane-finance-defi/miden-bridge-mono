...
use miden_objects::utils::{DeserializationError, Serializable};
use thiserror::Error;
use alloy_primitives::{Address, hex::{ FromHex, FromHexError }};
...
pub fn evm_address_to_felts<T>(address: T) -> Result<[Felt; 3], AddressFormatError> where T: AsRef<str>{

    let evm_dest_address = Address::from_hex(address.as_ref())
