...
use miden_objects::{account::AccountId, crypto::{rand::{FeltRng, RpoRandomCoin}, utils::word_to_hex}, AccountIdError, AssetError, Felt, FieldElement, NoteError, Word};
use miden_objects::asset::{Asset, FungibleAsset};
use miden_objects::note::{Note, NoteAssets, NoteExecutionHint, NoteFile, NoteDetails, NoteMetadata, NoteTag, NoteType};
...
fn build_recipient(receiver: String) -> Result<(), CliError> {
    ...
    let receiver = AccountId::from_hex(&receiver.as_str())
        .map_err(|e| CliError::ParseAccountIdError(e))?;
    ...
    let serial_number = word_to_hex(&serial_number)
        .map_err(|e| CliError::WordHexEncodingError(e))?;
...
fn restore_note(
    ...
    let faucet_id = AccountId::from_hex(&faucet_id.as_str())
        .map_err(|e| CliError::ParseAccountIdError(e))?;
    ...
    let note = Note::new(
        NoteAssets::new(vec![Asset::from(asset)]).map_err(|e| CliError::BuildExportableNoteError(e))?,
        NoteMetadata::new(
            faucet_id,
            NoteType::Private,
